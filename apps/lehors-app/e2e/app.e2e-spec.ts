...
describe('Starting tests for lehors-app', function() {
  ...
  it('website title should be lehors-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('lehors-app');
    })
  });

  it('network-name should be lehors-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('lehors-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be lehors-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('lehors-app');
    });
  });
